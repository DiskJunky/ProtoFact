...
| `ProtoFact.Scenarios` | `DemoScenario` - single source of truth for the demo recipe/item/goal graph and starting inventory, shared by both front-ends. |
| `ProtoFact.Console` | Entry point (`Program.cs`) that loads `DemoScenario` and runs the live loop; `RealTimeProvider` (`ITimeProvider` wall-clock impl); `UiRenderer` (Spectre.Console dashboard: system overview table, footer with goals/bottlenecks/system metrics, processor table). |
| `ProtoFact.Wpf` | WPF front-end (`net10.0-windows`) that loads the same `DemoScenario` and drives the same `ProductionController`/`Engine` tick loop via a `DispatcherTimer` on the UI thread. Shows a system-overview `DataGrid` plus a "Raw Resources" panel that lets the user add to any `ItemType.Raw` item's stock at runtime (recipes stay fixed). See [ROADMAP.md](./ROADMAP.md) item 2 for the architecture rationale. |
